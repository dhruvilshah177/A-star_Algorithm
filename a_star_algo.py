...
def main():
    ...
    while not pq.empty():
        ...
        for row, col, dir in zip(x, y, direction):
            ...
            if 0 <= x_current + row < 120 and 0 <= y_current + col < 160:
                ...
                temp_g_score = current[0] + get_cost(grid, x_current, y_current, x_current + row, y_current + col, dir)

                # if temp_g_score is less than the score we had for this node, set that score
                if temp_g_score < g_score[(row + x_current, col + y_current)]:
                    g_score[(row + x_current, col + y_current)] = temp_g_score

                    # to keep track of where we came from to this node - the path
                    came_from[(row + x_current, col + y_current)] = (x_current, y_current)

                    # f_score = g_score + h_score of the node
                    f_score[(row + x_current, col + y_current)] = g_score[(row + x_current, col + y_current)] + heuristic((row + x_current, col + y_current), goal)

                    # if the node we are on is not in priority queue and is not an obstacle, then add it to pq
                    if ((row + x_current, col + y_current) not in pq_set) and grid[row + x_current][
                        col + y_current] != "0":
                        pq.put((f_score[(row + x_current, col + y_current)], (row + x_current, col + y_current)))
                        pq_set.add((row + x_current, col + y_current))
    ...
    np.savetxt("A*_finished_map2.txt", grid, fmt="%s")
    #create_world code to send in starting coords
    file_name = "finished_map2.txt"
